        ...
        for line in self.file_obj:  # 每一行的最后一个符号必须是可以直接识别的
            ...
            while i < line_length:
                ...
                if self.state == 0:  # 初始状态
                    ...
                    if (i == line_length) and not (
                            self.last == 'begin' or self.last == 'end' or self.last == 'then' or self.last == 'do' or ch == '{' or ch == '}' or self.last == 'else'):  # 一行中最后的一个单个字符单词不可直接识别
...
if __name__ == '__main__':
    try:
        file_obj = open('input_file.txt', encoding='utf-8')
        dfa = DFA(file_obj)
